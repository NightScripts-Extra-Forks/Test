...
elseif state == gameState.TUTORIAL_STATE or state == gameState.TUTORIAL_EDIT_STATE or state == gameState.COURSE_EDIT_STATE then
...
if ui:isGlobalInputEnabled() == false then return end -- do not let to trigger main menu with 'Start' if ui input is blocked
...
if GlobalData:isBusy() == false then	
	if uiIsIdle == true or uiIsActive == false then
		if Input:isTriggered(BUTTON.START, uiIsActive) then -- force button trigger check
