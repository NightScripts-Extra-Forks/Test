...
-- hold play/edit buttons on release 
local button = ui:getScreen(SCREEN.BOTTOM):getButton("btn_play")
if button then
	button:setReleaseStay(true)
end
button = ui:getScreen(SCREEN.BOTTOM):getButton("btn_edit")
if button then
	button:setReleaseStay(true)
end
...
--Editor:lockInputUIFriendly() -- disable input now as it will take 1 frame to transition to next state
--Editor:lockInput(); -- will lock up the system if called
